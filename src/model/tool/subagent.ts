...
export class ListTemplates implements Tool<typeof ListTemplatesInputSchema, Array<{name: string; description: string}>> {
    static name: string = "listAgentTemplates";
    description = "List all available agent templates in the system.";
    inputSchema = ListTemplatesInputSchema;
    constructor() { }

    async handler(context: Context, session: Session, input: Record<string, never>): Promise<Array<{name: string; description: string}>> {
...
export class Spawn implements Tool<typeof SpawnInputSchema, string> {
    ...
    description: string = "Spawn an new agent based from a existing template (use the template name to specify a template you found with `listAgentTemplates`) and prompt. The agents will keep running on its own until they finish their given tasks, you will get the id of the spawned agent, with it you can use `await` tool to wait for it to complete and get the result.";
    ...
    async handler(context: Context, session: Session, input: z.infer<typeof SpawnInputSchema>): Promise<string> {
        ...
        if (!template) {
            throw new Error(`Agent template ${input.template_name} not found, please check the template_name parameter.`);
        }
        const spawnToolIndex = template.tools.findIndex(tool => tool.name === Spawn.name);
        if (spawnToolIndex !== -1) {
        ...
        newAgent.handleRequest(session, input.prompt);
        return newAgent.id;
    }
}
