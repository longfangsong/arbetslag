...
export class ListTemplates
	implements Tool<z.infer<typeof ListTemplatesInputSchema>, Output, never>
{
	...
	call(
		...
		return state.agentTemplateRepository
			.list()
			.then((templates: Array<{ name: string; description: string }>) => {
				return ok(
					templates.map((t: { name: string; description: string }) => ({
						name: t.name,
