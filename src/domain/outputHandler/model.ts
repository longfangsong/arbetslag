...
export function deserializeOutputHandler(serialized: SerializedOutputHandler, registry?: OutputHandlerRegistry): OutputHandler {
    switch (serialized.type) {
        case "user": {
            const handler = registry?.get(serialized.adapter, serialized.chat_id);
            if (!handler) {
                throw new Error(`Output handler not found for adapter=${serialized.adapter}, chat_id=${serialized.chat_id}`);
            }
            return handler;
        }
        case "to_parent":
