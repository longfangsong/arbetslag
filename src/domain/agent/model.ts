...
import { OutputHandler, ToParentOutputHandler, OutputHandlerRegistry, deserializeOutputHandler, serializeOutputHandler } from "../outputHandler/model";
...
export class Agent {
    ...
    private constructor(id: string, template: Template, outputHandler: OutputHandler, history: Array<HistoryEntry>) {
        this.id = id;
    ...
    static create(template: Template, outputHandler?: OutputHandler): Agent {
        return new Agent(nanoid(10), template, outputHandler ?? new ToParentOutputHandler("root"), []);
    }

    static async deserialize(data: SerializedAgent, templateRepo: TemplateRepository, registry?: OutputHandlerRegistry): Promise<Agent> {
        const template = await templateRepo.getByName(data.template_name);
        if (!template) {
            throw new Error(`Template not found: ${data.template_name}`);
        }
        const outputHandler = deserializeOutputHandler(data.output_handler, registry);
