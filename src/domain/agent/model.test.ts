...
describe("Agent serialization round-trip", () => {
	it("serializes and deserializes a root agent with UserOutputHandler", async () => {
		...
		expect(serialized.output_handler).toEqual({
			type: "user",
			adapter: "telegram",
			chat_id: "chat-123",
		});
		...
		const deserialized = await Agent.deserialize(
			serialized,
			templateRepo,
			registry,
		);
	...
	it("serializes and deserializes a sub-agent with ToParentOutputHandler", async () => {
		...
		expect(
			(deserialized.outputHandler as ToParentOutputHandler).parent_agent_id,
		).toBe("parent-456");
	});
	...
	it("deserializes with AgentRepository.deserialize", async () => {
		...
		const agent2 = Agent.create(
			template,
			new ToParentOutputHandler("parent-abc"),
		);
		...
		const deserializedRepo = await AgentRepository.deserialize(
			serialized,
			templateRepo,
			registry,
		);
