...
import { AgentMessageEvent, ApiCallbackEvent, LLMCompletionResponse, MessageEvent, ToolResponseEvent, Event } from "@/application/event/event";
...
export class Agent {
	...
	async handleMessage(event: MessageEvent): Promise<Array<Event>> {
		...
		await this.fs?.writeFile(`${this.agentDir}${this.id}.json`, JSON.stringify(this.serialize()));
		return [{
			id: nanoid(10),
	...
	async handleLLMCompletionResponse(event: LLMCompletionResponse): Promise<Array<Event>> {
		this.history.push({
			role: "assistant",
			content: event.content,
