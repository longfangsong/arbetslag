...
export async function dispatchEvent(
  ...
  switch (event.event_type) {
    case "message": {
      ...
      console.log(
        `[dispatch/message] chat_id=${message_event.chat_id}, content="${message_event.content}"`,
      );
      let agent = await agentRepository.getByChatId(message_event.chat_id);
      if (!agent) {
        console.log(
          `[dispatch/message] New agent created for chat ${message_event.chat_id}`,
        );
        const template = await templateRepository.default();
        console.log(
          `[dispatch/message] Template: name=${template?.name}, model=${template?.model}, ai_provider=${template?.ai_provider}`,
        );
    ...
    case "api_callback": {
      ...
      const agent = await agentRepository.getById(
        api_callback_event.to_agent_id!,
      );
      agent?.setPersistContext(fileSystem, agentRepository.dir);
    ...
    case "tool_call_request": {
      ...
      const tool = await toolRepository.getByName(
        tool_call_event.tool_call.tool_name,
      );
      const agent = await agentRepository.getById(
        tool_call_event.from_agent_id,
      );
      agent?.setPersistContext(fileSystem, agentRepository.dir);
      const result = await tool?.call(
        { fileSystem },
        agent!,
        tool_call_event.tool_call.arguments,
      );
      const content =
        result === undefined
          ? "Tool not found"
    ...
    case "tool_call_response": {
      ...
      console.log(
        `[dispatch] tool_call_response, id=${tool_call_response_event.id},
        event=${JSON.stringify(event)}`,
      );
      const agent = await agentRepository.getById(
        tool_call_response_event.to_agent_id,
      );
      agent?.setPersistContext(fileSystem, agentRepository.dir);
      console.log(`[dispatch] tool_call_response, agent=${agent?.id}`);
      return await agent?.handleToolResponse(tool_call_response_event);
    }
    case "agent_message": {
      let agent_message_event = event as AgentMessageEvent;
      const agent = await agentRepository.getById(
        agent_message_event.to_agent_id,
      );
      agent?.setPersistContext(fileSystem, agentRepository.dir);
    ...
    case "llm_completion_request": {
      ...
      const agent = await agentRepository.getById(
        llm_completion_request_event.from_agent_id,
      );
      agent?.setPersistContext(fileSystem, agentRepository.dir);
      ...
      console.log(
        `[dispatch/llm] agent=${agent?.id}, provider=${agent?.template.ai_provider}, model=${agent?.template.model}, history_len=${llm_completion_request_event.history.length}`,
      );
      console.log(
        `[dispatch/llm] history:`,
        JSON.stringify(llm_completion_request_event.history, null, 2).slice(
          0,
          500,
        ),
      );
      const completion_result = await aiProvider?.complete(
        agent!.template.model,
        llm_completion_request_event.history,
        toolRepository.tools,
      );
      console.log(
        `[dispatch/llm] completion_result: content_len=${completion_result?.content?.length ?? 0}, tool_calls=${completion_result?.tool_calls?.length ?? 0}`,
      );
    ...
    case "llm_completion_response": {
      ...
      const agent = await agentRepository.getById(
        llm_completion_response_event.to_agent_id,
      );
      agent?.setPersistContext(fileSystem, agentRepository.dir);
      return await agent?.handleLLMCompletionResponse(
        llm_completion_response_event,
      );
    }
    case "agent_output": {
      ...
      if (outputRouter) {
        console.log(
          `[dispatch/output] agent=${agent_output_event.from_agent_id}, content_len=${agent_output_event.content?.length ?? 0}`,
        );
